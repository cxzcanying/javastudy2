...
public class HospitalManager {
    ...
    //所有预约详情
    private ArrayList<Appointment> allAppointment=new ArrayList<>();
    private Scanner sc=new Scanner(System.in);
    public void start() {
        while (true) {
            System.out.println("====欢迎进入医院信息管理系统====");
                ...
                for (int j = 0; j < doctors.size(); j++) {
                    ...
                    for (int k = 0; k < schedules.size(); k++) {
                        ...
                        if(schedule.isMorning()){
                            System.out.println("上午"+schedule.getMstart()+"-"+schedule.getMend()+ " 总数/已预约 " +schedule.getmTotalNumber() + "/" + schedule.getmAppointNumber());

                        }else{
                            System.out.println("上午休息");
                        }
                        if(schedule.isAfternoon()){
                            System.out.println("下午"+schedule.getAstart()+"-"+schedule.getAend()+ " 总数/已预约 " +schedule.getaTotalNumber() + "/" + schedule.getaAppointNumber());
